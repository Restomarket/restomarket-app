...
@Injectable()
export class WarehousesRepository extends WarehousesRepositoryBase {
  constructor(@Inject(DATABASE_CONNECTION) db: DatabaseConnection, pinoLogger: PinoLogger) {
    ...
    super(db, warehouses, logger);
    pinoLogger.setContext(WarehousesRepository.name);
  }
}
