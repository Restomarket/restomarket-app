 ...
 */
@Injectable()
export class AgentRegistryRepository extends AgentRegistryRepositoryBase {
  constructor(@Inject(DATABASE_CONNECTION) db: DatabaseConnection, pinoLogger: PinoLogger) {
    ...
    super(db, agentRegistry, logger);
    pinoLogger.setContext(AgentRegistryRepository.name);
  }
}
